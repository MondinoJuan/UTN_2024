...
# Host     : localhost
# Port     : 3306
# Database : lopa_siemple_limpia


SET FOREIGN_KEY_CHECKS=0;

DROP DATABASE IF EXISTS `lopa_siemple_limpia`;

CREATE DATABASE `lopa_siemple_limpia`
    CHARACTER SET 'utf8'
    COLLATE 'utf8_general_ci';

USE `lopa_siemple_limpia`;
...
CREATE TABLE `tratamiento_limpieza` (
  ...
  CONSTRAINT `tratamiento_limpieza_fk1` FOREIGN KEY (`nro_servicio`) REFERENCES `servicios_limpieza` (`nro_servicio`) ON UPDATE CASCADE,
...
CREATE TABLE `tratamientos_procesos` (
  ...
  `cod_proceso` int(11) NOT NULL,
  `orden` int(11) NOT NULL,
  PRIMARY KEY  (`cod_tratamiento`,`orden`),
  KEY `tratamientos_procesos_procesos_fk` (`cod_proceso`),
  CONSTRAINT `tratamientos_procesos_fk` FOREIGN KEY (`cod_tratamiento`) REFERENCES `tratamientos` (`cod_tratamiento`),
...
INSERT INTO `clientes` (`tipo_doc`, `nro_doc`, `nom_ape`, `tel`, `dir`) VALUES 
  ('dni',11111111,'James Dewar','1111111','Lavalle 1111'),
  ('dni',22222222,'Mary Quant','2222222','Córdoba 2222'),
  ('dni',33333333,'King Camp Gillette','3333333','Lima 333'),
  ('dni',44444444,'Alexis Duchateau','4444444','Pasco 4444'),
  ('dni',55555555,'John Logie Baird','5555555','Paso de patos  55');
...
INSERT INTO `procesos_realizados` (`nro_servicio`, `orden`, `cod_proceso`, `fecha_inicio`, `hora_inicio`, `cuil_empleado`, `fecha_fin`, `hora_fin`, `resultado_proceso`) VALUES 
  ...
  (15,2,11,'2008-09-21','10:08:11','77-77777777-7','2008-09-21','10:26:11','Satisfactorio'),
  (15,2,5,'2008-09-21','10:44:05','77-77777777-7','2008-09-21','11:02:05','Satisfactorio');
